...
public class Create {
	 ...
	 * create a file inside the desired location. If the user does not geve the file a supported extention, 
	 * the the program will auto add to the end of it a .txt to convert it into a text file.
	 * 
	 * Unfortuanutly we did not add a case for is the user adds a file type at the end but we do not support it
	 * So you could end up with something like test.tocx.txt, which is an invalid file.
	 * In the future we would improve this argument so that even if you gave us a file type already, 
	 * the program would change it to one of the two valid types we set.
	 * 
	 * @param file_directory
	 * @param file_name
	 */
	public static void create_file(String file_directory, String file_name) {
		if (!file_name.contains(".txt") || !file_name.contains(".java")) { //this checks to see if the input it got has an extention or not
	 ...
	 * this method grabs all the content of that file and adds a class to the top of the file
	 * 
	 * This does not delete anything that is already in the file, all it does is add to the top of the file
	 * It also adds a public static void main(String[] args) to the file, so if a user wants to run the program, they are able to.
	 * 
	 * @param file_path
	 * @param class_name
	 */
	public static void creatr_class(String file_path, String class_name) {
		try {
			...
			fileContents.add(0,
					"public class " + class_name + "(){\n public static void main(String[] args) {\n\n} \n}"); //also adds the ability to run the program if the user wants to
	...
	/**
	 * This method when called ads a method to the selected file the user wants
	 * The method can be called whatever the user wants, but in order to even add this their must first be a class inside the file
	 * If there is no class, the program will not add anything to the file.
	 * @param File_Directory
	 * @param return_type
	 * @param method_name
	 */
	...
	/**
	 * This program will add content to any method the user has in the file
	 * We have it set up so that the when searching for a method name, it will remove () from it and add our own.
	 * This was added because if there are two methods called test and test2, it will reach both and add the content to both of them.
	 * The way we have it set up now will only search for the name + (), so that even if two methods are named similarly, it will be able to tell the difference between them
	 * @param Method_Name
	 * @param File_dir
	...
	/**
	 * This was added to help clean up the users search for a method name, as they might add () to the end of it by accident
	 * @param simple
	 * @return String
	 */
